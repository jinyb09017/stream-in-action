...
public class Problem4 {
    ...
    public static Map<String, List<Employee>> collect(List<Employee> employees) {
        ...
        Map<String, List<Employee>> orderMap = new HashMap<>();
        for (Entry<String, List<Employee>> stringListEntry : employeesMap.entrySet()) {
            List<Employee> sorted = stringListEntry.getValue().stream().sorted(Comparator.comparingInt(Employee::getAge)).collect(
                Collectors.toList());
            orderMap.put(stringListEntry.getKey(), sorted);
        }
        return orderMap;
